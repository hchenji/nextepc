...
status_t context_read_file() {
	...
	d_assert(file, return CORE_ERROR, "Failed to read configuration file `%s`",
			config->path);
		...
		if (!strcmp(root_key, "db_uri")) {
				...
				if (!strcmp(logger_key, "file")) {
					...
					while (yaml_iter_next(&network_iter)) {
						...
						if (!strcmp(network_key, "file")) {
							self.logger.network.file = yaml_iter_value(
									&network_iter);
						} else if (!strcmp(network_key, "unixDomain")) {
							self.logger.network.unix_domain = yaml_iter_value(
									&network_iter);
						} else
					...
					while (yaml_iter_next(&trace_iter)) {
						...
						if (!strcmp(trace_key, "app")) {
							...
							if (v)
								self.logger.trace.app = atoi(v);
						} else if (!strcmp(trace_key, "s1ap")) {
							const char *v = yaml_iter_value(&trace_iter);
							if (v)
								self.logger.trace.s1ap = atoi(v);
						} else if (!strcmp(trace_key, "nas")) {
							const char *v = yaml_iter_value(&trace_iter);
							if (v)
								self.logger.trace.nas = atoi(v);
						} else if (!strcmp(trace_key, "diameter")) {
							const char *v = yaml_iter_value(&trace_iter);
							if (v)
								self.logger.trace.diameter = atoi(v);
						} else if (!strcmp(trace_key, "gtpv2")) {
							const char *v = yaml_iter_value(&trace_iter);
							if (v)
								self.logger.trace.gtpv2 = atoi(v);
						} else if (!strcmp(trace_key, "gtp")) {
							const char *v = yaml_iter_value(&trace_iter);
							if (v)
								self.logger.trace.gtp = atoi(v);
						} else
			...
			while (yaml_iter_next(&parameter_iter)) {
				...
				if (!strcmp(parameter_key, "no_hss")) {
					self.parameter.no_hss = yaml_iter_bool(&parameter_iter);
				} else if (!strcmp(parameter_key, "no_sgw")) {
					self.parameter.no_sgw = yaml_iter_bool(&parameter_iter);
				} else if (!strcmp(parameter_key, "no_pgw")) {
					self.parameter.no_pgw = yaml_iter_bool(&parameter_iter);
				} else if (!strcmp(parameter_key, "no_pcrf")) {
					self.parameter.no_pcrf = yaml_iter_bool(&parameter_iter);
				} else if (!strcmp(parameter_key, "sctp_streams")) {
					const char *v = yaml_iter_value(&parameter_iter);
					if (v)
						self.parameter.sctp_streams = atoi(v);
				} else if (!strcmp(parameter_key, "no_ipv4")) {
					self.parameter.no_ipv4 = yaml_iter_bool(&parameter_iter);
				} else if (!strcmp(parameter_key, "no_ipv6")) {
					self.parameter.no_ipv6 = yaml_iter_bool(&parameter_iter);
				} else if (!strcmp(parameter_key, "prefer_ipv4")) {
					self.parameter.prefer_ipv4 = yaml_iter_bool(
							&parameter_iter);
				} else if (!strcmp(parameter_key, "multicast")) {
					self.parameter.multicast = yaml_iter_bool(&parameter_iter);
				} else if (!strcmp(parameter_key, "no_slaac")) {
					self.parameter.no_slaac = yaml_iter_bool(&parameter_iter);
				} else
 ...
 */
static bool context_mongoc_client_get_server_status(mongoc_client_t *client, /* IN */
...
status_t context_consul_init(const char *db_uri) {
	self.db_client = malloc(sizeof(consul_client_t));
...
status_t context_db_init(const char *db_uri) {
	...
	if (!context_mongoc_client_get_server_status(self.db_client, NULL, &reply,
			&error)) {
