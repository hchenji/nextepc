		...
		if (!strcmp(root_key, "hss")) {
			...
			while (yaml_iter_next(&hss_iter)) {
				...
				if (!strcmp(hss_key, "freeDiameter")) {
					yaml_node_t *node = yaml_document_get_node(document,
							hss_iter.pair->value);
					...
					if (node->type == YAML_SCALAR_NODE) {
						...
						while (yaml_iter_next(&fd_iter)) {
							...
							if (!strcmp(fd_key, "identity")) {
								self.fd_config->cnf_diamid = yaml_iter_value(
										&fd_iter);
							} else if (!strcmp(fd_key, "realm")) {
								self.fd_config->cnf_diamrlm = yaml_iter_value(
										&fd_iter);
							} else if (!strcmp(fd_key, "port")) {
								const char *v = yaml_iter_value(&fd_iter);
								if (v)
									self.fd_config->cnf_port = atoi(v);
							} else if (!strcmp(fd_key, "sec_port")) {
								const char *v = yaml_iter_value(&fd_iter);
								if (v)
									self.fd_config->cnf_port_tls = atoi(v);
							} else if (!strcmp(fd_key, "no_sctp")) {
								self.fd_config->cnf_flags.no_sctp =
										yaml_iter_bool(&fd_iter);
							} else if (!strcmp(fd_key, "listen_on")) {
								self.fd_config->cnf_addr = yaml_iter_value(
										&fd_iter);
								...
								do {
									...
									while (yaml_iter_next(&ext_iter)) {
										const char *ext_key = yaml_iter_key(
												&ext_iter);
									...
									if (module) {
										self.fd_config->ext[self.fd_config->num_of_ext].module =
												module;
										self.fd_config->ext[self.fd_config->num_of_ext].conf =
												conf;
								...
								do {
									...
									while (yaml_iter_next(&conn_iter)) {
										...
										if (!strcmp(conn_key, "identity")) {
											...
											d_warn("unknown key `%s`",
													conn_key);
									}

									if (identity && addr) {
										self.fd_config->conn[self.fd_config->num_of_conn].identity =
												identity;
										self.fd_config->conn[self.fd_config->num_of_conn].addr =
												addr;
										self.fd_config->conn[self.fd_config->num_of_conn].port =
												port;
...
//    		d_print("Consul initialized\n");
		...
		self.subscriberCollection = mongoc_client_get_collection(
				context_self()->db_client, context_self()->db_name,
				"subscribers");
...
//	see if imsi exists
	...
	//	iterate thru the linked list consisting of kv pairs
	consul_kv_t *tmp = ll;
	while (tmp != NULL) {
...
status_t hss_db_auth_info(char *imsi_bcd, hss_db_auth_info_t *auth_info) {
	if (context_self()->use_consul) {
...
status_t hss_db_update_rand_and_sqn(char *imsi_bcd, c_uint8_t *rand,
		c_uint64_t sqn) {
	...
	query = BCON_NEW("imsi", BCON_UTF8(imsi_bcd));
	update = BCON_NEW("$set", "{", "security.rand", printable_rand,
			"security.sqn", BCON_INT64(sqn), "}");

	if (!mongoc_collection_update(self.subscriberCollection, MONGOC_UPDATE_NONE,
...
status_t hss_db_increment_sqn(char *imsi_bcd) {
	...
	query = BCON_NEW("imsi", BCON_UTF8(imsi_bcd));
	update = BCON_NEW("$inc", "{", "security.sqn", BCON_INT64(32), "}");
	if (!mongoc_collection_update(self.subscriberCollection, MONGOC_UPDATE_NONE,
			query, update, NULL, &error)) {
		d_error("mongoc_collection_update() failure: %s", error.message);
	...
	update = BCON_NEW("$bit", "{", "security.sqn", "{", "and",
			BCON_INT64(max_sqn), "}", "}");
	if (!mongoc_collection_update(self.subscriberCollection, MONGOC_UPDATE_NONE,
			query, update, NULL, &error)) {
		d_error("mongoc_collection_update() failure: %s", error.message);
...
status_t hss_db_subscription_data_consul(char *imsi_bcd,
	...
	while (tmp != NULL) {
		...
		if (!strcmp(key + len - 23, "access_restriction_data")) {
			subscription_data->access_restriction_data = strtol(tmp->val, NULL,
					10);
		} else if (!strcmp(key + len - 17, "subscriber_status")) {
			subscription_data->subscriber_status = strtol(tmp->val, NULL, 10);
		} else if (!strcmp(key + len - 19, "network_access_mode")) {
			subscription_data->network_access_mode = strtol(tmp->val, NULL, 10);
			...
			subscription_data->ambr.downlink = strtol(tmp->val, NULL, 10)
					* 1024;
		} else if (!strcmp(key + len - 3, "pdn")) {
			strcpy(pdn_list, tmp->val);
		}
...
status_t hss_db_subscription_data(char *imsi_bcd,
		s6a_subscription_data_t *subscription_data) {
	...
	while (bson_iter_next(&iter)) {
		...
		BSON_ITER_HOLDS_INT32(&iter)) {
			subscription_data->access_restriction_data = bson_iter_int32(&iter);

		} else if (!strcmp(key, "subscriber_status") &&
		BSON_ITER_HOLDS_INT32(&iter)) {
			subscription_data->subscriber_status = bson_iter_int32(&iter);
		} else if (!strcmp(key, "network_access_mode") &&
		BSON_ITER_HOLDS_INT32(&iter)) {
			subscription_data->network_access_mode = bson_iter_int32(&iter);
		} else if (!strcmp(key, "subscribed_rau_tau_timer") &&
		BSON_ITER_HOLDS_INT32(&iter)) {
			subscription_data->subscribed_rau_tau_timer = bson_iter_int32(
					&iter);
			...
			while (bson_iter_next(&child1_iter)) {
				...
				BSON_ITER_HOLDS_INT64(&child1_iter)) {
					subscription_data->ambr.uplink = bson_iter_int64(
							&child1_iter) * 1024;
				} else if (!strcmp(child1_key, "downlink") &&
				BSON_ITER_HOLDS_INT64(&child1_iter)) {
					subscription_data->ambr.downlink = bson_iter_int64(
							&child1_iter) * 1024;
				}
			}
		} else if (!strcmp(key, "pdn") && BSON_ITER_HOLDS_ARRAY(&iter)) {
			int pdn_index = 0;
			...
			while (bson_iter_next(&child1_iter)) {
				...
				d_assert(pdn_index < MAX_NUM_OF_SESS, goto out,
						"Overflow of PDN number(%d>%d)", pdn_index,
						MAX_NUM_OF_SESS);
				...
				while (bson_iter_next(&child2_iter)) {
					...
					BSON_ITER_HOLDS_DOCUMENT(&child2_iter)) {
						...
						while (bson_iter_next(&child3_iter)) {
							const char *child3_key = bson_iter_key(
									&child3_iter);
							...
							BSON_ITER_HOLDS_DOCUMENT(&child3_iter)) {
								...
								while (bson_iter_next(&child4_iter)) {
									const char *child4_key = bson_iter_key(
											&child4_iter);
					...
					BSON_ITER_HOLDS_DOCUMENT(&child2_iter)) {
						...
						while (bson_iter_next(&child3_iter)) {
							...
							BSON_ITER_HOLDS_INT64(&child3_iter)) {
								pdn->ambr.uplink = bson_iter_int64(&child3_iter)
										* 1024;
					...
					BSON_ITER_HOLDS_DOCUMENT(&child2_iter)) {
						...
						while (bson_iter_next(&child3_iter)) {
							...
							BSON_ITER_HOLDS_UTF8(&child3_iter)) {
								ipsubnet_t ipsub;
								const char *v = bson_iter_utf8(&child3_iter,
										&length);
							...
							BSON_ITER_HOLDS_UTF8(&child3_iter)) {
								...
								if (rv == CORE_OK) {
									pdn->pgw_ip.ipv6 = 1;
									memcpy(pdn->pgw_ip.both.addr6, ipsub.sub,
											sizeof(ipsub.sub));
								}
							}
						}
